...
v_max = 12e3;
lb = [-(1.5*r_E)*ones(3*N,1); -v_max*ones(3*N,1);-u_max*ones(3*N,1)];
ub = [(1.5*r_E)*ones(3*N,1); v_max*ones(3*N,1); u_max*ones(3*N,1)];

options=optimoptions('fmincon','Display','iter','Algorithm','sqp','MaxFunctionEvaluations',10000,'MaxIterations',10000);
